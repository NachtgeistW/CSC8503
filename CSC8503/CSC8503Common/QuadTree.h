	...
	namespace CSC8503 {
		...
		protected:
			...
			void Insert(T& object, const Vector3& objectPos, const Vector3& objectSize, int depthLeft, int maxSize) {
				if (!CollisionDetection::AABBTest(objectPos, Vector3(position.x, 0, position.y), objectSize, Vector3(size.x, 10000.0f, size.y)))
					return;
				if(children)
				{
				...
				{
					// currently a leaf node , can expand
