...
bool CollisionDetection::RayBoxIntersection(const Ray& r, const Vector3& boxPos, const Vector3& boxSize, RayCollision& collision) {
    ...
    const float bestT = tVals.GetMaxElement();
    if (bestT < 0.0f)
        return false;
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        //best intersection doesn't touch the box
...
bool CollisionDetection::RayAABBIntersection(const Ray& r, const Transform& worldTransform, const AABBVolume& volume, RayCollision& collision) {
    const Vector3 boxPos = worldTransform.GetPosition();
    const Vector3 boxSize = volume.GetHalfDimensions();
    return RayBoxIntersection(r, boxPos, boxSize, collision);
}

bool CollisionDetection::RayOBBIntersection(const Ray& r, const Transform& worldTransform, const OBBVolume& volume, RayCollision& collision) {
    const Quaternion orientation = worldTransform.GetOrientation();
    const Vector3 position = worldTransform.GetPosition();

    const auto transform = Matrix3(orientation);
    const auto invTransform = Matrix3(orientation.Conjugate());

    const Vector3 localRayPos = r.GetPosition() - position;

    const Ray tempRay(invTransform * localRayPos, invTransform * r.GetDirection());
    const bool collided = RayBoxIntersection(tempRay, Vector3(), volume.GetHalfDimensions(), collision);
    if (collided)
...
bool CollisionDetection::RaySphereIntersection(const Ray& r, const Transform& worldTransform, const SphereVolume& volume, RayCollision& collision) {
    const Vector3 spherePos = worldTransform.GetPosition();
    ...
    const Vector3 point = r.GetPosition() + r.GetDirection() * sphereProj;
    const float sphereDist = (point - spherePos).Length();
...
bool CollisionDetection::ObjectIntersection(GameObject* a, GameObject* b, CollisionInfo& collisionInfo) {
    ...
    VolumeType pairType = (VolumeType)(static_cast<int>(volA->type) | static_cast<int>(volB->type));
...
bool CollisionDetection::AABBTest(const Vector3& posA, const Vector3& posB, const Vector3& halfSizeA, const Vector3& halfSizeB) {
    const auto delta = posB - posA;
    const auto totalSize = halfSizeA + halfSizeB;
...
bool CollisionDetection::AABBIntersection(const AABBVolume& volumeA, const Transform& worldTransformA,
    ...
    const auto boxAPos = worldTransformA.GetPosition();
    const auto boxBPos = worldTransformB.GetPosition();

    const auto boxASize = volumeA.GetHalfDimensions();
    const auto boxBSize = volumeB.GetHalfDimensions();

    //Test if they are overlapping
...
bool CollisionDetection::SphereIntersection(const SphereVolume& volumeA, const Transform& worldTransformA,
    ...
    float radii = volumeA.GetRadius() + volumeB.GetRadius();//半径
    ...
    {
        float penetration = radii - deltaLength;//穿透程度
        auto normal = delta.Normalised();//法线
        auto localA = normal * volumeA.GetRadius();
        auto localB = -normal * volumeB.GetRadius();
...
bool CollisionDetection::AABBSphereIntersection(const AABBVolume& volumeA, const Transform& worldTransformA,
    ...
    auto delta = worldTransformB.GetPosition() - worldTransformA.GetPosition();
    auto closestPointOnBox = Maths::Clamp(delta, -boxSize, boxSize);
    auto localPoint = delta - closestPointOnBox;
    float distance = localPoint.Length();

    //Collided
    if (distance < volumeB.GetRadius())
    {
        auto collisionNormal = localPoint.Normalised();//碰撞法线
        float penetration = volumeB.GetRadius() - distance;//穿透程度
